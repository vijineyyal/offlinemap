...
import 'package:offlinemap/constants.dart';
import 'package:offlinemap/main.dart';
import 'package:offlinemap/offline_map.dart';
...
final LatLngBounds kerala = LatLngBounds(
  northeast: LatLng(10.675572, 76.136490),
  southwest: LatLng(10.635076, 76.109338),
);
...
final List<OfflineRegionDefinition> regionDefinitions = [
  ...
  OfflineRegionDefinition(
    ...
    minZoom: 15.0,
    maxZoom: 18.0,
    mapStyleUrl: 'mapbox://styles/mapbox/satellite-streets-v12',
  ),
];

final List<String> regionNames = ['Hawaii', 'Santiago', 'Auckland', 'Kerala'];
...
final List<OfflineRegionListItem> allRegions = [
  ...
  OfflineRegionListItem(
    offlineRegionDefinition: regionDefinitions[3],
    downloadedId: null,
    isDownloading: false,
    name: regionNames[3],
    estimatedTiles: 202,
...
class _OfflineRegionsBodyState extends State<OfflineRegionBody> {
        ...
        ListView.builder(
            ...
            itemBuilder: (context, index) {
              print(_items[index].name +
                  "-Download Status" +
                  _items[index].isDownloaded.toString());
              return Row(
  ...
  void _updateListOfRegions() async {
    List<OfflineRegion> offlineRegions =
        await getListOfRegions(accessToken: token);
  ...
  final Function(DownloadRegionStatus event) onEvent =
      (DownloadRegionStatus status) async {
    if (status.runtimeType == Success) {
      print("SUCCESSSSSS");
      var snackBar = SnackBar(content: Text('Map Downloaded Successfully'));
      ...
      print("Progresss :" + progress.toString());
      ...
      print("ERRORRRRRRR");
      var snackBar =
          SnackBar(duration: Duration(seconds: 2), content: Text('Error...'));
      ScaffoldMessenger.of(NavigationService.navigatorKey.currentContext!)
          .showSnackBar(snackBar);
    }
  };
  ...
  void _downloadRegion(OfflineRegionListItem item, int index) async {
    ...
    setOfflineTileCountLimit(6000000);
    try {
      final downloadingRegion =
          await downloadOfflineRegion(item.offlineRegionDefinition,
              metadata: {
